                                        ...
                                        byte* out, word32 outSz, void* heap)
{
    ...
    tmpASz = (word32)sizeof(tmpA) / (word32)sizeof(tmpA[0]);
    /* if tmp is not large enough than use some dynamic memory */
    if (((seedSz + 4U) > tmpASz) || ((word32)hLen > tmpASz)) {
        ...
        tmpSz = ((seedSz + 4U) > (word32)hLen)? seedSz + 4U: (word32)hLen;
    ...
    do {
        ...
        tmp[seedSz]      = (byte)(counter >> 24) & 0xFFU;
        tmp[seedSz + 1U] = (byte)(counter >> 16) & 0xFFU;
        tmp[seedSz + 2U] = (byte)(counter >>  8) & 0xFFU;
        tmp[seedSz + 3U] = (byte)(counter)       & 0xFFU;

        /* hash and append to existing output */
        if ((ret = wc_Hash(hType, tmp, (seedSz + 4U), tmp, tmpSz)) != 0) {
        ...
        void* heap)
{
    ...
    if ((ret = wc_Hash(hType, optLabel, labelLen, lHash, (word32)hLen)) != 0) {
    ...
    idx = pkcsBlockLen - 1U - inputLen;
    psLen = (int)pkcsBlockLen - (int)inputLen - 2 * hLen - 2;
    ...
    idx = idx - (word32)hLen + 1U;
    XMEMCPY(pkcsBlock + idx, lHash, (word32)hLen);

    /* generate random seed */
    if ((ret = wc_RNG_GenerateBlock(rng, seed, (word32)hLen)) != 0) {
        #ifdef WOLFSSL_SMALL_STACK
            XFREE(lHash, heap, DYNAMIC_TYPE_RSA_BUFFER);
            XFREE(seed,  heap, DYNAMIC_TYPE_RSA_BUFFER);
        #endif
    ...
    dbMask = (byte*)XMALLOC((size_t)pkcsBlockLen - (size_t)hLen - (size_t)1,
    ...
    /* help static analyzer */
    XMEMSET(dbMask, 0, ((size_t)pkcsBlockLen - (size_t)hLen - (size_t)1));

    ret = RsaMGF(mgf, seed, (word32)hLen, dbMask, pkcsBlockLen - (word32)hLen - 1U, heap);
    if (ret != 0) {
    ...
    if ((ret = RsaMGF(mgf, pkcsBlock + (word32)hLen + 1U, pkcsBlockLen - (word32)hLen - 1U,
                                           pkcsBlock + 1U, (word32)hLen, heap)) != 0) {
        #ifdef WOLFSSL_SMALL_STACK
            XFREE(lHash, heap, DYNAMIC_TYPE_RSA_BUFFER);
            XFREE(seed,  heap, DYNAMIC_TYPE_RSA_BUFFER);
        #endif
        ...
        int saltLen, int bits, void* heap)
{
    ...
    m += inputLen;
    if ((ret = wc_RNG_GenerateBlock(rng, salt, (word32)saltLen)) != 0) {
        return ret;
    }
    XMEMCPY(m, salt, (word32)saltLen);
    m += saltLen;
    ...
    ret = RsaMGF(mgf, h, (word32)hLen, pkcsBlock,
            pkcsBlockLen - (word32)hLen - 1U, heap);
    if (ret != 0) {
        return ret;
    }
                           ...
                           word32 pkcsBlockLen, byte padValue, WC_RNG* rng)
{
    ...
    if (padValue == (byte)RSA_BLOCK_TYPE_1) {
        ...
        /* pad with 0xff bytes */
        XMEMSET(&pkcsBlock[1], 0xFF, (size_t)pkcsBlockLen - (size_t)inputLen -
                (size_t)2U);
    }
    else {
#ifndef WOLFSSL_RSA_VERIFY_ONLY
        ...
        padLen = pkcsBlockLen - inputLen - 1U;
        ...
        /* remove zeros */
        for (i = 1; i < padLen; i++) {
            if (pkcsBlock[i] == 0U) { pkcsBlock[i] = 0x01; }
        }
#else
        (void)rng;
    ...
    pkcsBlock[pkcsBlockLen-inputLen-1U] = 0U;     /* separator */
    XMEMCPY(pkcsBlock+pkcsBlockLen-inputLen, input, inputLen);
                            ...
                            byte* optLabel, word32 labelLen, void* heap)
{
    ...
    /* find seedMask value */
    if ((ret = RsaMGF(mgf, pkcsBlock + ((word32)hLen + 1U),
                            pkcsBlockLen - (word32)hLen - 1U, tmp, (word32)hLen, heap)) != 0) {
        XFREE(tmp, heap, DYNAMIC_TYPE_RSA_BUFFER);
        return ret;
    }
    ...
    for (idx = 0; idx < (word32)hLen; idx++) {
        tmp[idx] = tmp[idx] ^ pkcsBlock[1U + idx];
    }

    /* get dbMask value */
    if ((ret = RsaMGF(mgf, tmp, (word32)hLen, tmp + (word32)hLen,
                                       pkcsBlockLen - (word32)hLen - 1U, heap)) != 0) {
        XFREE(tmp, NULL, DYNAMIC_TYPE_RSA_BUFFER);
        return ret;
    }

    /* get DB value by doing maskedDB xor dbMask */
    for (idx = 0; idx < (pkcsBlockLen - (word32)hLen - 1U); idx++) {
        pkcsBlock[(word32)hLen + 1U + idx] = pkcsBlock[(word32)hLen + 1U + idx] ^ tmp[idx + (word32)hLen];
    }
     ...
     */
    check = 0;
    check = (word32)ConstantCompare(pkcsBlock + (word32)hLen + 1U, h, hLen);
    check = check + ((word32)pkcsBlock[idx] ^ 0x01U); /* separator value is 0x01 */
    idx = idx + 1U;
    check = check + ((word32)pkcsBlock[0]   ^ 0x00U); /* Y, the first value, should be 0 */

    /* Return 0 data length on error. */
    idx = (word32)ctMaskSelInt(ctMaskEq((int)check, 0), (int)idx,
            (int)pkcsBlockLen);

    /* adjust pointer to correct location in array and return size of M */
    *output = (byte*)(pkcsBlock + idx);
    return (int)pkcsBlockLen - (int)idx;
}
#endif /* WC_NO_RSA_OAEP */
                        ...
                        int saltLen, int bits, void* heap)
{
    ...
    if (pkcsBlock[pkcsBlockLen - 1U] != (byte)RSA_PSS_PAD_TERM) {
    ...
    if ((ret = RsaMGF(mgf, pkcsBlock + pkcsBlockLen - 1U - (word32)hLen,
                    (word32)hLen, tmp, pkcsBlockLen - 1U - (word32)hLen,
                    heap)) != 0) {
        XFREE(tmp, heap, DYNAMIC_TYPE_RSA_BUFFER);
        return ret;
    }
    ...
    *output = &pkcsBlock[pkcsBlockLen - ((word32)hLen + (word32)saltLen + 1U)];
    return saltLen + hLen;
}
#endif
...
/* UnPad plaintext, set start to *output, return length of plaintext,
 * < 0 on error */
static int RsaUnPad(byte *pkcsBlock, unsigned int pkcsBlockLen,
        ...
        *output = (byte*)(pkcsBlock + i);
        ...
        for (i = 0, j = 2; j < pkcsBlockLen; j++) {
           /* Update i if not passed the separator and at separator. */
            i |= (~pastSep) & (word16)ctMask16Eq((int)pkcsBlock[j], 0x00) & (j + 1U);
            pastSep |= (word16)ctMask16Eq((int)pkcsBlock[j], 0x00);
        }

        /* Minimum of 11 bytes of pre-message data - including leading 0x00. */
        invalid |= (byte)ctMaskLT((int)i, (int)RSA_MIN_PAD_SZ);
        /* Must have seen separator. */
        invalid |= (byte)(~pastSep);
        /* First byte must be 0x00. */
        invalid |= (byte)ctMaskNotEq((int)pkcsBlock[0], 0x00);
        /* Check against expected block type: padValue */
        invalid |= (byte)ctMaskNotEq((int)pkcsBlock[1], (int)padValue);

        /* using len with & result for MISRA C compliance */
        *output = (pkcsBlock + i);
        len = (((word32)~invalid) & (word32)(pkcsBlockLen - i));
        ret = (int)len;
    }
#endif
                          ...
                          word32* outLen, int type, RsaKey* key, WC_RNG* rng)
{
    ...
    if (mp_count_bits(&key->n) == 2048) {
        switch(type) {
#ifndef WOLFSSL_RSA_PUBLIC_ONLY
        case RSA_PRIVATE_DECRYPT: /* FALLTHROUGH */
        case RSA_PRIVATE_ENCRYPT:
    #ifdef WC_RSA_BLINDING
            if (rng == NULL) {
    ...
    #endif
    #ifndef RSA_LOW_MEM
            ret = sp_RsaPrivate_2048(in, inLen, &key->d, &key->p, &key->q,
            ...
            break;
#endif
        case RSA_PUBLIC_ENCRYPT: /* FALLTHROUGH */
        case RSA_PUBLIC_DECRYPT:
            ret = sp_RsaPublic_2048(in, inLen, &key->e, &key->n, out, outLen);
            break;
        default:
            ret = RSA_WRONG_TYPE_E;
            break;
    ...
    if (mp_count_bits(&key->n) == 3072) {
        switch(type) {
    ...
    #endif
    #ifndef RSA_LOW_MEM
            ret = sp_RsaPrivate_3072(in, inLen, &key->d, &key->p, &key->q,
                                      &key->dP, &key->dQ, &key->u, &key->n,
                                      out, outLen);
    #else
            ret = sp_RsaPrivate_3072(in, inLen, &key->d, &key->p, &key->q,
            ...
            case RSA_PUBLIC_ENCRYPT:
            case RSA_PUBLIC_DECRYPT:
                ret = sp_RsaPublic_3072(in, inLen, &key->e, &key->n, out, outLen);
                break;
            default:
                ret = RSA_WRONG_TYPE_E;
                break;
    ...
    if (ret == 0) {
        if (mp_read_unsigned_bin(tmp, in, (int)inLen) != MP_OKAY) {
            ret = MP_READ_E;
        }
    }
    ...
    if (ret == 0) {
        ...
        #endif   /* WC_RSA_BLINDING */
        #else
            WOLFSSL_MSG("RSA Private key operations not compiled in");
            ret = RSA_WRONG_TYPE_E;
        #endif
        }
    ...
    if (ret == 0) {
        *outLen = keyLen;
        if (mp_to_unsigned_bin_len(tmp, out, (int)keyLen) != MP_OKAY) {
             ret = MP_TO_E;
        }
    }
#else
                          ...
                          word32* outLen, int type, RsaKey* key, WC_RNG* rng)
{
    ...
    if (type == RSA_PRIVATE_DECRYPT &&
        ...
        if (ret == 0) {
            if (mp_read_unsigned_bin(c, in, (int)inLen) != 0) {
                ret = MP_READ_E;
            }
        }
        if (ret == 0) {
        ...
        if (rsa_type == RSA_PUBLIC_DECRYPT && ret > (int)outLen) {
            ...
            if (outPtr == NULL) {
                ...
                for (i = 0U, j = 0U; j < key->dataLen; j++) {
                    ...
                    d = d & (byte)ctMaskLT((int)i, (int)outLen);
                    c = (signed char)d;
                    /* 0 - no add, -1 add */
                    i += (word32)((byte)(-c));
                }
#else
                XMEMCPY(out, pad, (word32)ret);
#endif
            }
            else {
                *outPtr = pad;
            ...
            /* Error with RSA unpadding */
            WOLFSSL_MSG("Error with RSA unpad");
        }
...
{
    return wc_RsaPSS_CheckPadding_ex(in, inSz, sig, sigSz, hashType, (int)inSz, 0);
}
                              ...
                              int saltLen, int bits)
{
    ...
    if (inSz != (word32)wc_HashGetDigestSize(hashType)) {
        ret = BAD_FUNC_ARG;
    }
    ...
    if (ret == 0) {
        if (saltLen == -1) {
            ...
            #ifdef WOLFSSL_SHA512
                /* See FIPS 186-4 section 5.5 item (e). */
                if (bits == 1024 && inSz == (word32)WC_SHA512_DIGEST_SIZE) {
                    saltLen = RSA_PSS_SALT_MAX_SZ;
                }
            #endif
        }

        if (saltLen < 0 || (word32)saltLen > inSz) {
            ret = PSS_SALTLEN_E;
        }
    }
    ...
    if (ret == 0) {
        /* sanity check that size is not larger than sigCheck buffer */
        if ((word32)RSA_PSS_PAD_SZ + inSz + (word32)saltLen >
                (word32)WC_MAX_DIGEST_SIZE * 2U + (word32)RSA_PSS_PAD_SZ) {
            WOLFSSL_MSG("Unexpected size of saltLen and input");
            ret = BAD_FUNC_ARG;
        }
        else {
            ...
            XMEMCPY(sigCheck + RSA_PSS_PAD_SZ + inSz, sig, (word32)saltLen);
            ret = wc_Hash(hashType, sigCheck, (word32)RSA_PSS_PAD_SZ +
                    inSz + (word32)saltLen, sigCheck, inSz);
        }
    }
    if (ret == 0) {
                          ...
                          int nlen, int* isPrime, WC_RNG* rng)
{
    ...
    if ((qRaw != NULL && qRawSz == 0U) || (qRaw == NULL && qRawSz != 0U)) {
        return BAD_FUNC_ARG;
    }
    ...
    if (ret == MP_OKAY) {
        ret = mp_read_unsigned_bin(&p, pRaw, (int)pRawSz);
    }

    if (ret == MP_OKAY) {
        if (qRaw != NULL) {
            ret = mp_read_unsigned_bin(&q, qRaw, (int)qRawSz);
            if (ret == MP_OKAY) {
                Q = &q;
            }
        }
    ...
    if (ret == MP_OKAY) {
        ret = mp_read_unsigned_bin(&e, eRaw, (int)eRawSz);
    }
    ...
    ret = (ret == MP_OKAY) ? 0 : (int)PRIME_GEN_E;
...
int wc_MakeRsaKey(RsaKey* key, int size, long e, WC_RNG* rng)
{
    ...
    if (err == MP_OKAY) {
        buf = (byte*)XMALLOC((word32)primeSz, key->heap, DYNAMIC_TYPE_RSA);
        if (buf == NULL) {
            err = MEMORY_E;
        }
    }
    ...
    if (err == MP_OKAY) {
        ...
        do {
...
#endif
            /* generate value */
            err = wc_RNG_GenerateBlock(rng, buf, (word32)primeSz);
            if (err == 0) {
            ...
            (void)i;
#endif
        } while (err == MP_OKAY && isPrime == 0 && i < failCount);
    }

    if (err == MP_OKAY && isPrime == 0) {
        err = PRIME_GEN_E;
    }
    ...
    if (err != 0) {
        if (wc_FreeRsaKey(key) != 0) {
            WOLFSSL_MSG("Error freeing RSA key");
            /* already in error state go to return err */
        }
        return err;
    }
