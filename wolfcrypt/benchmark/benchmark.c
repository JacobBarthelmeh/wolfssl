                                   ...
                                   int countSz, double start, int ret)
{
    ...
    if (csv_format == 1) {
        ...
        desc, BENCH_ASYNC_GET_NAME(useDeviceID), blocks, blockType, word[0], total, word[1],
        persec, blockType);
    ...
    /* Add to thread stats */
    bench_stats_add(BENCH_STAT_SYM, NULL, 0, desc, useDeviceID, persec, blockType, ret);
    ...
    const char* desc, int useDeviceID, int count, double start, int ret)
{
    ...
    /* Add to thread stats */
    bench_stats_add(BENCH_STAT_ASYM, algo, strength, desc, useDeviceID, opsSec, kOpsSec, ret);
...
#ifdef WOLFSSL_SECO_CAAM
    if (wc_SECO_OpenHSM(2, 0x7777, 10000, 1) != 0) {
